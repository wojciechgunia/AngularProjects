...
@Component({
  ...
  providers: [{provide: NG_VALUE_ACCESSOR, useExisting: PhoneControlComponent, multi: true}]
})
export class PhoneControlComponent implements ControlValueAccessor, OnDestroy{
  numberPrefixControl = new FormControl('',[Validators.required,Validators.pattern("[+]{0,1}[0-9]{1,2}")]);
  numberControl = new FormControl('',[Validators.required,Validators.pattern("[0-9 ]{9,11}")]);
  ...
  getError()
  {
    if ((this.numberPrefixControl.hasError('required') || this.numberControl.hasError('required')) && !(this.numberPrefixControl.hasError('pattern') || this.numberControl.hasError('pattern')))
